...
The TMC2209 chip has an enable not input pin that switches off the power stage,
...
pull the enable not input pin high, which causes the driver to be disabled by
default.

To hardware enable the driver, pull the enable not pin low, either with a jumper
or with an output pin from the microcontroller.

This library does not control an enable output, but the method
disabledByInputPin() can be used to tell if the driver is disabled in hardware.
