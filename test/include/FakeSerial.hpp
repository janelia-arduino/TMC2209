...
public:
  ...
  void set_register_value(uint8_t register_address, uint32_t value)
  {
...
private:
  ...
  static uint8_t crc8_tmc_(const uint8_t *bytes, size_t count_without_crc)
  {
    uint8_t crc = 0;
  ...
  static uint32_t reverse_u32_(uint32_t data)
  {
  ...
  void maybe_handle_read_request_(const std::vector<uint8_t> &frame)
  {
